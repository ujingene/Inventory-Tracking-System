...
//display all item
function track_all_item() {
	$.ajax({
		url: '../data/track_assigned_item.php',
		type: 'post',
		async: false,
		success: function (event) {
			$('#track').html(event);
...
//display all item
function assign_inventory() {
	$.ajax({
		url: '../data/issue_item.php',
		type: 'post',
		async: false,
		success: function (event) {
			$('#assignInventory').html(event);
		},
		error: function () {
			alert('Error: Issue Error 504');
		}
	});
...
function track_avilable_item() {
	$.ajax({
		url: '../data/track_available_item.php',
		type: 'post',
		async: false,
		success: function (event) {
			$('#available').html(event);
		},
		error: function () {
			alert('Error: Track Error 504');
		}
...
/*
*Assign and Update Inventory to PSA Employees
*/

//Load assign_item_modal function in Available.php
...
//Load assign_item_modal function in Available.php
function assign_item_modal(iID) {
	$.ajax({
		url: '../data/item_profile.php',
		type: 'post',
		dataType: 'json',
		...
		success: function (data) {
			...
			$('#modal-available-item').modal('show');
		},
		error: function () {
			alert('Error: assign_item_modal L172+');
		}
	});
