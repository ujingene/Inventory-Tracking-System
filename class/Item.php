...
class Item extends Database implements iItem{
	...
	{	
		$sql="UPDATE tbl_item
			  SET 
			  item_name = ?, item_serno = ?, item_modno = ?, item_brand = ?, item_amount = ?, item_purdate = ?, emp_id = ?, 
			  cat_id = ?, 
			  con_id = ?
			  WHERE item_id = ?
		";
	...
	public function item_report($choice)
	{
		...
		if($choice == 'all'){
			$sql = "SELECT *
					FROM tbl_item i 
					INNER JOIN tbl_item_assigned a
